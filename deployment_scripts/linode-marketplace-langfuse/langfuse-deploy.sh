...
function obj_check {
	# Object Storage is all-or-nothing: a partial set of fields fails fast here rather
	# than silently falling back to MinIO against the user's intent.
	local set_count=0
...
EOF
	...
	if [[ -n ${OBJ_BUCKET} ]]; then
		# normalize endpoint: allow pasted scheme/trailing slash; region = first endpoint label
		...
		echo "[info] No Object Storage fields set; using bundled MinIO"
	fi
...
function run {
	# object storage fields are all-or-nothing; validate before any work
	obj_check
