...
## Usage
```
/review:validate-config [app-name] [--targets PATH...] [--instance IP]
```

## Arguments
- `app-name`: Name of the marketplace app (e.g., `linode-marketplace-hashicorp-nomad`).
- `--targets PATH...`: One or more config templates to validate. Defaults to every `*.j2`, `*.conf`, `*.hcl` under `apps/<app-name>/roles/`.
...
## Worked Example: HashiCorp Nomad Backport (April 2026)

The Nomad marketplace app's nginx template inherited four custom directives. Empirical validation produced this matrix:
...
| `proxy_set_header Origin` | DEAD CODE (Nomad 2.0 doesn't enforce Origin) |

The two DEAD CODE directives were removed in `fc9c8c2d`. Test logs and full evidence are in the PR description and `~/.claude/projects/.../memory/project_nomad_validation_findings.md`.
