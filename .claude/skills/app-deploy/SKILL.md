...
also what `/review:validate-config` (Phase 5) validates against.
...
**Then — mirror, push, redeploy:**
   ...
   g. After the push, create a **fresh** box from the updated branch (Phase 4a) and re-monitor.
      A from-scratch deploy must succeed with **no** VM-side edits — that's the real pass condition.
...
6. Leave the box **up** (target for `/review:validate-config`). Update `STATE.md`: mark `deploy`
   done, record stackscript id + box id/ip, set `next_step: /review:validate-config`.
...
**Next:** `/review:validate-config <app> --instance <deploy-box-ip>`
