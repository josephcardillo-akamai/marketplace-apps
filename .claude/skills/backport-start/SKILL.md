...
```
Parse `--stackscript`, `--region`, `--type` from `$ARGUMENTS`; `$app` is the first positional.
...
5. `git clone` the upstream app repo into `.reference/<repo>` (gitignored scratch). Record the
