...
load-bearing, mark it for `/review:validate-config` rather than dropping or cargo-culting it.
...
| `collections.yml` | Copy, then **update version pins** for what this app needs |
...
7. Lint via the **committed wrapper scripts** (canonical per `AGENTS.md` — they apply the repo's
   ...
   ```
8. **Clean the lint scratch before any `git add`:** `env` and `.ansible` are intentionally NOT
...
- [ ] Local lint clean and the scratch dirs (`env`, `.ansible`) are removed (with permission).
